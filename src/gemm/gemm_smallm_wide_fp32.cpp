...
static void gemm_wide_2x48(int K,
                            ...
                            int j0, int j_len,
                            float alpha, float beta) {
    // Process in 16-col sub-blocks to fit registers
    for (int js = 0; js < j_len; js += kSubBlockN) {
        int sub_len = std::min(kSubBlockN, j_len - js);
        int sub_full4 = (sub_len / 4) * 4;

        // 2 rows × 4 acc each = 8 accumulators
        ...
        for (; k + 3 < K; k += 4) {
            // K iteration 0
            {
                ...
                if (sub_len > 4)  { b1 = vld1q_f32(bk + 4);  r0c1 = vfmaq_n_f32(r0c1, b1, a0); r1c1 = vfmaq_n_f32(r1c1, b1, a1); }
                if (sub_len > 8)  { b2 = vld1q_f32(bk + 8);  r0c2 = vfmaq_n_f32(r0c2, b2, a0); r1c2 = vfmaq_n_f32(r1c2, b2, a1); }
                if (sub_len > 12) { b3 = vld1q_f32(bk + 12); r0c3 = vfmaq_n_f32(r0c3, b3, a0); r1c3 = vfmaq_n_f32(r1c3, b3, a1); }
            }
            // K iteration 1
            {
                const float* bk = B + (k+1) * ldb + j0 + js;
                float a0 = A0[k+1], a1 = A1[k+1];
                float32x4_t b0 = vld1q_f32(bk);
                r0c0 = vfmaq_n_f32(r0c0, b0, a0); r1c0 = vfmaq_n_f32(r1c0, b0, a1);
                if (sub_len > 4)  { float32x4_t b = vld1q_f32(bk+4);  r0c1 = vfmaq_n_f32(r0c1, b, a0); r1c1 = vfmaq_n_f32(r1c1, b, a1); }
                if (sub_len > 8)  { float32x4_t b = vld1q_f32(bk+8);  r0c2 = vfmaq_n_f32(r0c2, b, a0); r1c2 = vfmaq_n_f32(r1c2, b, a1); }
                if (sub_len > 12) { float32x4_t b = vld1q_f32(bk+12); r0c3 = vfmaq_n_f32(r0c3, b, a0); r1c3 = vfmaq_n_f32(r1c3, b, a1); }
            }
            // K iteration 2
            {
                const float* bk = B + (k+2) * ldb + j0 + js;
                float a0 = A0[k+2], a1 = A1[k+2];
                float32x4_t b0 = vld1q_f32(bk);
                r0c0 = vfmaq_n_f32(r0c0, b0, a0); r1c0 = vfmaq_n_f32(r1c0, b0, a1);
                if (sub_len > 4)  { float32x4_t b = vld1q_f32(bk+4);  r0c1 = vfmaq_n_f32(r0c1, b, a0); r1c1 = vfmaq_n_f32(r1c1, b, a1); }
                if (sub_len > 8)  { float32x4_t b = vld1q_f32(bk+8);  r0c2 = vfmaq_n_f32(r0c2, b, a0); r1c2 = vfmaq_n_f32(r1c2, b, a1); }
            ...
            // K iteration 3
            {
                const float* bk = B + (k+3) * ldb + j0 + js;
                float a0 = A0[k+3], a1 = A1[k+3];
                float32x4_t b0 = vld1q_f32(bk);
                r0c0 = vfmaq_n_f32(r0c0, b0, a0); r1c0 = vfmaq_n_f32(r1c0, b0, a1);
                if (sub_len > 4)  { float32x4_t b = vld1q_f32(bk+4);  r0c1 = vfmaq_n_f32(r0c1, b, a0); r1c1 = vfmaq_n_f32(r1c1, b, a1); }
                if (sub_len > 8)  { float32x4_t b = vld1q_f32(bk+8);  r0c2 = vfmaq_n_f32(r0c2, b, a0); r1c2 = vfmaq_n_f32(r1c2, b, a1); }
...
static void gemm_wide_4x16(int K,
                            ...
                            float alpha, float beta) {
    // 4 rows × 4 acc each = 16 accumulators
    float32x4_t r0c0 = vdupq_n_f32(0), r0c1 = vdupq_n_f32(0);
    float32x4_t r0c2 = vdupq_n_f32(0), r0c3 = vdupq_n_f32(0);
    float32x4_t r1c0 = vdupq_n_f32(0), r1c1 = vdupq_n_f32(0);
    float32x4_t r1c2 = vdupq_n_f32(0), r1c3 = vdupq_n_f32(0);
...
static void gemm_wide_MxN(int M, int K,
    ...
    // Allocate stack-backed accumulators: M rows × ceil(j_len/4) SIMD vectors
    int n_acc = (j_len + 3) / 4;
    ...
    // K-loop: stream B once, all M rows share B access
    for (int k = 0; k < K; ++k) {
...
void gemm_smallm_wide_driver_fp32(int M, int N, int K,
    ...
    // Remaining single row: use existing M=1 GEMV path
    if (i < M) {
        // Inline M=1 wide GEMV (same as gemm_1xn_fp32 / gemm_smallm1_fp32)
